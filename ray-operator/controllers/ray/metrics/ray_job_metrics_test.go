...
func TestMetricRayJobInfo(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			manager := NewRayJobMetricsManager(context.Background(), client, testRayJobMetricTTL)
...
func TestRayJobCleanupLoop(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		manager.startRayJobCleanupLoop(ctx, 2*time.Second)
	}()

	// Schedule a cleanup for a job
	manager.metricTTL = 2 * time.Second // Set TTL to 2 seconds for testing
